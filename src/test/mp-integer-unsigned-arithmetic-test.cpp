...
void test_rs_core_mp_integer_unsigned_arithmetic() {
    ...
    TRY(s = y.str());    TEST_EQUAL(s, "15");
    TRY(s = y.str(16));  TEST_EQUAL(s, "f");

    TRY(y = x - 15u);
    TEST_EQUAL(y.bits(), 0u);
    TRY(s = y.str());    TEST_EQUAL(s, "0");
    TRY(s = y.str(16));  TEST_EQUAL(s, "0");
    ...
    TRY(s = y.str());  TEST_EQUAL(s, "18446744073709551615");

    TRY(z = x + 15u);
    TEST_EQUAL(z.bits(), 61u);
    TRY(s = z.str());    TEST_EQUAL(s, "1311768467463790335");
    TRY(s = z.str(16));  TEST_EQUAL(s, "123456789abcdeff");

    TRY(z = x + y);
    TEST_EQUAL(z.bits(), 65u);
    TRY(s = z.str());    TEST_EQUAL(s, "19758512541173341935");
    TRY(s = z.str(16));  TEST_EQUAL(s, "1123456789abcdeef");

    TRY(z = y - 15u);
    TEST_EQUAL(z.bits(), 64u);
    TRY(s = z.str());    TEST_EQUAL(s, "18446744073709551600");
    TRY(s = z.str(16));  TEST_EQUAL(s, "fffffffffffffff0");

    TRY(z = 15u - y);
    TEST_EQUAL(z.bits(), 0u);
    TRY(s = z.str());    TEST_EQUAL(s, "0");
    TRY(s = z.str(16));  TEST_EQUAL(s, "0");

    TRY(z = y - x);
    TEST_EQUAL(z.bits(), 64u);
    TRY(s = z.str());    TEST_EQUAL(s, "17134975606245761295");
    TRY(s = z.str(16));  TEST_EQUAL(s, "edcba9876543210f");

    TRY(z = x * y);
    TEST_EQUAL(z.bits(), 125u);
    TRY(s = z.str());    TEST_EQUAL(s, "24197857203266734862169780735577366800");
...
void test_rs_core_mp_integer_unsigned_bit_operations() {
    ...
    TRY(z = x & y);
    TEST_EQUAL(z.bits(), 61u);
    TRY(s = z.str());    TEST_EQUAL(s, "1311768467463790320");
    TRY(s = z.str(16));  TEST_EQUAL(s, "123456789abcdef0");

    TRY(z = x | y);
    TEST_EQUAL(z.bits(), 64u);
    TRY(s = z.str());    TEST_EQUAL(s, "18446744073709551615");
