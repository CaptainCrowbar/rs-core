...
void test_rs_core_root_finding_bisection() {
    ...
    TEST_THROW(x = rf.solve(0, 0, -1), std::domain_error, "Invalid arguments for bisection");
    TRY(rf.set_limit(10));
    TEST_THROW(x = rf.solve(0, 0, 1), std::range_error, "Root finder failed to converge");
...
void test_rs_core_root_finding_false_position() {
    ...
    auto f = [] (double x) { return x * x * x - 2 * x + 0.25; };
    auto rf = false_position<double>(f);
    double x = 0;
    double eps = 1e-10;
    ...
    TEST_THROW(x = rf.solve(0, 0, -1), std::domain_error, "Invalid arguments for false position");
    TRY(rf.set_limit(10));
    TEST_THROW(x = rf.solve(0, 0, 1), std::range_error, "Root finder failed to converge");
